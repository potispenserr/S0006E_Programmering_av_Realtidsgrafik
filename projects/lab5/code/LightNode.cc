...
LightNode::LightNode()
{
	...
	lightCube.loadObj("./resources/CoronaFixed.obj");
...
void LightNode::bindLighting()
{
	...
	texRes.loadTex("./resources/BotellaText.jpg");
	...
	model = model * Matrix4D::scale(Vector4D(0.05f, 0.05f, 0.05f));
	

	lightCube.updateTransform(lightCube.getTransform().scale(Vector4D(0.2f, 0.2f, 0.2f)));
	lightCube.setTransform(Matrix4D::scale((Vector4D(0.02f, 0.02f, 0.02f))));
}
...
void LightNode::render(Camera cam, Matrix4D projection)
{
	...
	lightCube.updateTransform(Matrix4D::scale(Vector4D(0.02f, 0.02f, 0.02f)));
	...
	glBindTexture(GL_TEXTURE_2D, texRes.texID);
